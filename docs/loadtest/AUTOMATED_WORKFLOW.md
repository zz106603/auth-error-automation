...
- DM-001: domain-mix demo script, `TARGET_RPS`, `DEMO_DURATION`
...
DM-001은 capacity test가 아니라 MCP/도메인 통계 demo다. 다양한 auth failure taxonomy payload를 유입해 `auth_error_hourly_type_stats`, `auth_error_context_distribution`, `auth_error_cluster_summary`에 의미 있는 분포가 쌓이는지 확인한다. 자세한 해석은 `docs/loadtest/DM-001-domain-mix.md`를 따른다.
