...
class OutboxRetryAndDeadIntegrationTest extends AbstractIntegrationTest {
    ...
    void 발행_실패_후_재시도_시간이_미래인_경우_즉시_재폴링되지_않고_시간이_지나야_재처리된다() {
        ...
        OutboxMessage afterFail = repo.findById(m.getId()).orElseThrow();
        ...
        assertThat(afterFail.getNextRetryAt()).isAfter(OffsetDateTime.now(ZoneOffset.UTC));
    ...
    void 최대_재시도_횟수에_도달하면_DEAD_상태로_전환된다() {
        ...
        // then
        OutboxMessage reloaded = repo.findById(m.getId()).orElseThrow();
        ...
        assertThat(reloaded.getRetryCount()).isEqualTo(10);
