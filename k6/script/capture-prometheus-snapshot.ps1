...
$queries = @(
  ...
  [ordered]@{ id = "publish_failure_rps"; type = "range"; unit = "rps"; query = 'sum(rate(auth_error_publish_total{event_type="auth.error.recorded.v1",result!="success"}[1m])) or vector(0)' },
  [ordered]@{ id = "publish_timeout_rps"; type = "range"; unit = "rps"; query = 'sum(rate(auth_error_publish_total{event_type="auth.error.recorded.v1",result="timeout"}[1m])) or vector(0)' },
  [ordered]@{ id = "publish_error_rps"; type = "range"; unit = "rps"; query = 'sum(rate(auth_error_publish_total{event_type="auth.error.recorded.v1",result="error"}[1m])) or vector(0)' },
