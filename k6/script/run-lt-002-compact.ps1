param(
  ...
  [string]$ScriptPathInContainer = "/scripts/lt_002_rampup_compact.js",
  [string]$TestId = ("LT-002C-" + (Get-Date -Format "yyyy-MM-dd_HHmmss")),
  ...
  [string]$Profile = "local-single-node-compact",
...
& $workflow `
  ...
  -StdoutLogName "lt_002_rampup_compact.stdout.log" `
  -K6SummaryFilePrefix "lt_002_rampup_compact" `
  -Network $Network `
  -EnvFile $EnvFile `
