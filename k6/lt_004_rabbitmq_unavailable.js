...
const RUN_ID = __ENV.TEST_ID || `LT-004B-${Date.now()}`;
...
const FAULT_START_DELAY_SEC = parsePositiveNumber(__ENV.FAULT_START_DELAY_SEC, 60, "FAULT_START_DELAY_SEC");
const RABBIT_DOWN_DURATION_SEC = parsePositiveNumber(__ENV.RABBIT_DOWN_DURATION_SEC, 60, "RABBIT_DOWN_DURATION_SEC");
...
export const options = {
  scenarios: {
    rabbitmq_unavailable: {
      ...
      preAllocatedVUs: 120,
      maxVUs: 600,
      tags: { lt: "LT-004B", phase: "rabbitmq-unavailable" },
    },
  },
};
...
export function setup() {
  console.log(
    `[FAULT_WINDOW_READY] scenario=LT-004B target_rps=${TARGET_RPS} fault_start_delay_sec=${FAULT_START_DELAY_SEC} rabbit_down_duration_sec=${RABBIT_DOWN_DURATION_SEC}`
  );
}
...
export default function () {
  ...
  const payload = JSON.stringify({
    ...
    userAgent: "k6",
    userId: `rabbit-down-user-${__VU}-${__ITER}-${rand}`,
  ...
  const headers = {
    ...
    "X-LT-Name": "LT-004B",
    "X-LT-Stage": `rabbitmq-unavailable-${TARGET_RPS}`,
...
function buildSummary(data) {
  ...
  const lines = [];
  lines.push("# LT-004B RabbitMQ Unavailable Summary");
  lines.push(`test_id=${__ENV.TEST_ID || "LT-004B"}`);
  lines.push(`generated_at=${new Date(endMs).toISOString()}`);
  lines.push(`duration_ms=${runMs}`);
  lines.push("");
  lines.push("[rabbitmq_unavailable]");
  lines.push(
    `[WINDOW] target_rps=${TARGET_RPS} duration=${STEADY_DURATION} fault_start_delay_sec=${FAULT_START_DELAY_SEC} rabbit_down_duration_sec=${RABBIT_DOWN_DURATION_SEC} timestamp=${new Date(startMs).toISOString()}`
  );
  lines.push("");
  lines.push("[summary]");
...
export function handleSummary(data) {
  ...
  const testId = __ENV.TEST_ID || `LT-004B-${new Date().toISOString().replace(/[:.]/g, "-")}`;
  const fileName = `lt_004_rabbitmq_unavailable-${testId}.log`;
