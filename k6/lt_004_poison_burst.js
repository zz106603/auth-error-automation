...
const RUN_ID = __ENV.TEST_ID || `LT-004D-${Date.now()}`;
...
const TARGET_RPS = parsePositiveNumber(__ENV.TARGET_RPS, 10, "TARGET_RPS");
const STEADY_DURATION = __ENV.STEADY_DURATION || "4m";
const POISON_TOTAL = parsePositiveNumber(__ENV.POISON_TOTAL, 80, "POISON_TOTAL");

export const options = {
  scenarios: {
    poison_burst_normal_traffic: {
      ...
      preAllocatedVUs: 80,
      maxVUs: 300,
      tags: { lt: "LT-004D", phase: "poison-burst-normal-traffic" },
    },
  },
};
...
export default function () {
  ...
  const payload = JSON.stringify({
    ...
    userAgent: "k6",
    userId: `poison-burst-normal-user-${__VU}-${__ITER}-${rand}`,
  ...
  const headers = {
    ...
    "X-LT-Name": "LT-004D",
    "X-LT-Stage": `poison-burst-${TARGET_RPS}`,
    "X-LT-Target-RPS": String(TARGET_RPS),
    "X-LT-Poison-Total": String(POISON_TOTAL),
  };
  ...
  const res = http.post(url, payload, {
    ...
    tags: {
      api: "auth-errors",
      lt: "LT-004D",
      phase: "poison-burst-normal-traffic",
...
function buildSummary(data) {
  ...
  const lines = [];
  lines.push("# LT-004D Poison Message Burst Summary");
  lines.push(`test_id=${__ENV.TEST_ID || "LT-004D"}`);
  lines.push(`generated_at=${new Date(endMs).toISOString()}`);
  lines.push(`duration_ms=${runMs}`);
  lines.push("");
  lines.push("[poison_burst_normal_traffic]");
  lines.push(
    `[WINDOW] target_rps=${TARGET_RPS} duration=${STEADY_DURATION} poison_total=${POISON_TOTAL} timestamp=${new Date(startMs).toISOString()}`
  );
  lines.push("");
  lines.push("[summary]");
...
export function handleSummary(data) {
  ...
  const testId = __ENV.TEST_ID || `LT-004D-${new Date().toISOString().replace(/[:.]/g, "-")}`;
  const fileName = `lt_004_poison_burst-${testId}.log`;
